-- =============================================================
-- 0014 — First-come-first-served seat assignment.
--
-- The public no longer picks specific seats from a map — they pick a
-- quantity (1-4) and the server assigns the next sellable seats in
-- row/col order. create_pending_order_by_qty mirrors
-- create_pending_order (see schema.sql) but selects its own seats
-- instead of trusting a client-supplied seat_ids array.
--
-- `for update skip locked` on the seat selection is what makes this
-- actually first-come-first-served under concurrency: two buyers
-- racing for the same show each lock a *different* set of the next
-- available rows instead of blocking on (or double-booking) the same
-- ones — the second one just skips whatever the first already locked
-- and picks up from the next free seat.
-- =============================================================
